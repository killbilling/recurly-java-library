...
public class TestNotification extends TestModelBase {
    ...
    private static Logger log = LoggerFactory.getLogger(TestNotification.class);
    ...
    private <T extends Notification> void deserialize(Class<T> clazz) {

        String xmlElement = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, clazz.getSimpleName());
        ...
        String notificationData = notificationDataBuilder.toString();
        log.debug("Test deserialization of \n{}", notificationData);

        Notification.Type detected = Notification.detect(notificationData);
        Assert.assertEquals(detected.getJavaType(), clazz);

        T notification = Notification.read(notificationData, clazz);
    ...
    private void testPaymentNotification(PaymentNotification paymentNotification) {
        ...
        PushTransaction.VerificationResult cvv = transaction.getCvvResult();
        Assert.assertNotNull(cvv);
        Assert.assertEquals(cvv.getCode(), "");
        Assert.assertNull(cvv.getMessage());

        PushTransaction.VerificationResult avs = transaction.getAvsResult();
